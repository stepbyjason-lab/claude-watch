...
- **Dense white-text decks:** if subtle text-only slides still merge, run `--scene-threshold 0.15 --phash-dist 2`. A lower detection floor surfaces faint changes as candidates and the edge-hash dedup keeps the noise bounded — validated to lift a 38-min white deck from 25 → 31 captured slides without ballooning. (Do not raise this to the global default; it is a deck-specific recommendation.)
