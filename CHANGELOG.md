...
- **Slides end-of-video tail anchor** — `apply_coverage_floor(..., include_tail_anchor=True)` (slides mode opts in; classic mode keeps the upstream default, byte-identical) guarantees one extractable floor near `duration − 0.5s`, so a slide shown only in the final seconds — past the last coverage step and too late to register as a scene cut — is still captured. Validated on a 38-min white deck (last slide recovered, 25 → 26); for dense text decks `--scene-threshold 0.15 --phash-dist 2` lifts capture further (25 → 31) without ballooning. Evidence: [`docs/dogfood/2026-06-05-harness-dhash-validation.md`](docs/dogfood/2026-06-05-harness-dhash-validation.md).
