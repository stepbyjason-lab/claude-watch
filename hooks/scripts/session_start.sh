...
case "$STATUS" in
  ready|"") exit 0 ;;
  needs_install) echo "[claude-watch] Run /watch — setup will install ffmpeg/yt-dlp on first run." ;;
  ...
  needs_install_and_key) echo "[claude-watch] Setup incomplete: run /watch and Claude will walk you through install + API key." ;;
esac
